...
stargazer(
  ...
  multinom_model6, multinom_model7,
...
# Score to beat
# Model 2 BIC: 1051.77 / Model 7 BIC: 1088.50 

Vars<-c(main_indip,econo,cof_deve,geo_colonial)
complete_formula <-gen_formula(dependent_var,Vars)
multinom_modela <- multinom(complete_formula, data = df_scaled)

Vars<-c(main_indip,econo,cof_deve,geo_colonial,"dis_count") 
complete_formula <-gen_formula(dependent_var,Vars)
multinom_modelb <- multinom(complete_formula, data = df_scaled)

# Model B Best GDP per capita model, with model 7 as a maximum model 
# (no alternate controls considered yet)

screenreg(list(multinom_model2,multinom_modela,multinom_modelb,multinom_model7))
...
complete_formula <-gen_formula(dependent_var,Vars) 
# Estimate the multinomial logistic regression model
mod1 <- multinom(complete_formula, data = df_scaled,
                            Hess = TRUE)
...
ggplot(data = pred1$plotdata, aes(x = ln_gdp_per_cap_ppp_lag_1, 
                                  y = mean,
                                  ymin = lower, ymax = upper)) +
  geom_ribbon(alpha = 0.1) + # Confidence intervals
  geom_line() + # Mean
  ...
  labs(y = "Predicted probabilities",
       x = "GDP per capita") # Always label your axes ;)
