...
df_Aiddata<- df_Aiddata %>% 
  group_by(iso3c, year ) %>%
  summarize(commitment_tot = sum(AidData_tot_commit),
            commitment_tot_pc = sum(AidData_tot_commit_per_cap))
...
#change names
df_Aiddata2$recipient[df_Aiddata$recipient=="Yugoslavia"]            <- "Serbia"
df_Aiddata2$recipient[df_Aiddata$recipient=="Serbia and Montenegro"] <- "Serbia"
df_Aiddata2$recipient[df_Aiddata$recipient=="Soviet Union"]          <- "Russia"
...
#### Calculate number of costs per year 
```{r}
...
process_EmDat_fat <- function(data) {
  ...
  # Select variables of interest
  data <- data[c("DisNo", "StartYear", "EndYear", "Country", "DisasterSubgroup", "TotalDamageAdjusted000US")]
  ...
  # Subset variables
  data <- data[c("iso3c", "DisasterSubgroup", "EndYear","TotalDamageAdjusted000US")]
  

     # List variables
  List <-  c("TotalDamageAdjusted000US")
  # # Assign 0s
  # data <- data %>%
  #   mutate(across(all_of(List), ~ replace_na(., 0)))
  ...
  data <- data %>%
  group_by(iso3c, EndYear) %>%
  summarise(DamAd = sum(TotalDamageAdjusted000US))

    names(data)[names(data) == "EndYear"] <- "year"
    names(data)[names(data) == "DamAd"] <- "DamAd"
...
# Apply function for different thresholds
EM_dat_cost <- process_EmDat_fat(EMDAT)
...
## add cost Mdata
#df_Ind_var <- left_join(df_Ind_var, EM_dat_cost, by = c("year" = "year", "iso3c" = "iso3c"))
## add KOF index
...
## create path
folder_path <- file.path(path,"df_Ind_var_v3.rds")
## save file
