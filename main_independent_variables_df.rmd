...
df_donors_nonstate<-df_Aiddata[is.na(df_Aiddata$iso3c_donor), ]
  ...
  df_donors_state<- df_donors_state %>% 
    group_by(iso3c, year) %>%
    summarize(N_donors_state = n_distinct(donor))



  df_donors_nonstate<- df_donors_nonstate %>% 
    group_by(iso3c, year) %>%
    summarize(N_donors_nonstate = n_distinct(donor))
...
df_donors<- left_join(df_donors_state, df_donors_nonstate, by = c("year" = "year", "iso3c" = "iso3c", "country" = "country"))
...
# Drop country
df_EMDAT_full <- select(df_EMDAT_full, -country)
```

# Merge datasets main independent and controls
```{r}
## add world bank variables
df_Ind_var <- left_join(df_UNcomtrade_main, df_WorldBank, by = c("year" = "year", "iso3c" = "iso3c","country" = "country"))
## add humanitarian dfs
df_Ind_var <- left_join(df_Ind_var, df_HOD_cy, by = c("year" = "year", "iso3c" = "iso3c","country" = "country"))
## add vdem indexes
df_Ind_var <- left_join(df_Ind_var, df_Vdem_s, by = c("year" = "year", "iso3c" = "iso3c","country" = "country"))
## add conf events
df_Ind_var <- left_join(df_Ind_var, df_UCDP_GED_Y, by = c("year" = "year", "iso3c" = "iso3c","country" = "country"))
## add membership
df_Ind_var <- left_join(df_Ind_var, df_IGOs, by = c("year" = "year", "iso3c" = "iso3c","country" = "country"))
## add donors
df_Ind_var <- left_join(df_Ind_var, df_donors, by = c("year" = "year", "iso3c" = "iso3c","country" = "country"))
...
#folder_path_wb <- file.path(desktop_path, main_folder_name, sub_folder_name, namefile)
folder_path_wb <-"C:/Users/krogers/Nextcloud/Global Solidarity/Compiled/df_Ind_var.rds"
...
df_Ind_var<-readRDS(paste0(userpath,"Nextcloud/Global Solidarity/Compiled/df_ind_var.rds"))
