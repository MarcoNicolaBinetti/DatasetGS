...
process_AID_2 <- function(df_input) {
  ...
  # Initialize lists to store rolling averages
  means_Hum_plus_ERaid   <- c()
  means_extended_aid_tot <- c()

  # Loop through each unique year in the dataset
  for (year in unique(df_input$year)) {
    # Calculate the rolling average for Hum_plus_ER_aid_tot
    hum_plus_mean           <- mean(df_input$Hum_plus_ER_aid_tot[df_input$year <= year], na.rm = TRUE)
    means_Hum_plus_ERaid    <- c(means_Hum_plus_ERaid, hum_plus_mean)

    # Calculate the rolling average for extended_aid_tot
    extended_mean           <- mean(df_input$extended_aid_tot[df_input$year <= year], na.rm = TRUE)
    means_extended_aid_tot  <- c(means_extended_aid_tot, extended_mean)
  }

  # Create a combined data frame with the results
  means_df <- data.frame(
    year = unique(df_input$year),
    year_mean_Hum_plus_ERaid   = means_Hum_plus_ERaid,
    year_mean_extended_aid_tot = means_extended_aid_tot
  )

  # Replace values with NA for years after 2013
  means_df[means_df$year > 2013, c("year_mean_Hum_plus_ERaid", "year_mean_extended_aid_tot")] <- NA
...
df_AID_dis100k_y_small <- process_AID_2(df_AID_EMDAT_100ky)
...
df_AID_dis100k_y  <- process_AID_3(df_AID_dis100k_y_small,df_AID_EMDAT_100ky)
...
# Code GlobSol 5k
```{r}
df5k= left_join(df_ICWES_dis5k_y, df_aid_sec_joined, by = c("year" = "year", "iso3c" = "iso3c"))
...
# Code GlobSol 50k
```{r}
df50k= left_join(df_ICWES_dis50k_y, df_aid_sec_joined, by = c("year" = "year", "iso3c" = "iso3c"))
# drop NA agreement (keep only country years with disasters)
...
# Code GlobSol 100k
```{r}
df100k= left_join(df_ICWES_dis100k_y, df_aid_sec_joined, by = c("year" = "year", "iso3c" = "iso3c"))
# drop NA agreement (keep only country years with disasters)
...
## create path
folder_path <- file.path(path,"globsol_df50k.rds")
## save file
saveRDS(df50k,folder_path)

## create path
folder_path <- file.path(path,"globsol_df5k.rds")
## save file
saveRDS(df5k,folder_path)

## create path
folder_path <- file.path(path,"globsol_df100k.rds")
## save file
