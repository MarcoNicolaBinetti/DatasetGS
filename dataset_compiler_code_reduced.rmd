...
## create and unzip new data folder
...
df_Polity2<-subset(df_Polity2, df_Polity2$year>1994 & df_Polity2$year<2019)
...
check_duplicates<-df_Base_Y %>% group_by(year,iso3c) %>%
...
## set list of months
Date<-as.data.frame(seq(as.Date("1995-1-1"), as.Date("2018-12-31"), by = "month"))
...
# drop countries pre independence
...
process_EmDat_1 <- function(data, min_affected) {
  ...
  # Subset years
  data <- subset(data, StartYear > 1993)
  
  # Subset only big disasters
  data <- subset(data, TotalAffected > min_affected)
  ...
  # Subset variables
  data <- data[c("DisNo", "iso3c", "DisasterSubgroup", "Date.start", "Date.end","TotalDeaths")]
...
check <- EM_dat_50k %>%
  mutate(endyear=year(Date.end),
         cy=paste0(iso3c,endyear)) %>%
  filter(endyear>1994,endyear<2019)
check1 <- unique(check$cy) # 863 disaster-endyears ( all territories)
...
# List unique disasters
dis_list <- df %>% 
    group_by(iso3c, date_month=as.yearmon(date)) %>%
  summarise(dis_ids = list(sort(unique(DisNo))), .groups = "drop")
...
# Join disaster list
df <- left_join(df, dis_list, by=c("iso3c","date_month"))
...
summary(EM_dat_50k_processed_2)
...
process_EmDat_3 <- function(df, df_basey) {
  ...
  # Summarise: disaster dummy by type / list and count disasters per year
  df <- df %>%
    group_by(iso3c, year) %>%
    summarise(
      across(ends_with("_dummy"), sum),
      dis_ids = list(unique(na.omit(unlist(dis_ids)))),
      dis_count = n_distinct(na.omit(unlist(dis_ids))),
      .groups = "drop"
    )
  
  # create dummy variables
  List <- setdiff(names(df), c("iso3c", "year", "dis_ids", "dis_count"))
  ...
  # Assign 0s to dummy variables (but not to dis_ids_year or dis_count_unique)
  df <- df %>%
    mutate(across(all_of(List), ~ replace_na(., 0)))
  
  # Add zeros
  df <- df %>%
    mutate(dis_ids = if_else(sapply(dis_ids, length) == 0, 
                              list(NA_character_), 
                              dis_ids),
...
summary(EM_dat_50k_processed_3)

check2 <- nrow(EM_dat_50k_processed_3 %>% mutate(cy=paste0(iso3c,year)) %>% filter(dis_count>0)) # 912 CY disasters occurring (states only)

```

#### Sum deaths per country-year 
```{r, include=FALSE}

process_EmDat_4 <- function(df, emdat_dv) {
  
  # spread deaths across years for multi-year disasters
  df_1 <- df %>%
  group_by(iso3c, DisNo, TotalDeaths) %>%
  summarise(year = list(seq(floor_date(min(Date.start), unit = "year"),
                            floor_date(max(Date.end), unit = "year"),
                            by = "year"))) %>%
  unnest(cols = c(year)) %>%
  mutate(year = as.numeric(year(year)),
         n_years = n(),
         year_deaths = TotalDeaths / n_years, na.rm=T) %>%
  ungroup() %>%
    group_by(iso3c,year) %>%
    summarise(dis_death=round(sum(year_deaths,na.rm=T),0))
  
  # List disasters ending eachyear
  df_2<-df %>%
     group_by(iso3c, year=year(Date.end)) %>%
     summarise(end_dis_ids = list(sort(unique(DisNo))), .groups = "drop")

  #Join end list
  df_1 <- left_join(df_1, df_2, by = c("year", "iso3c"))
  
  #Join to Emdat DV
  df_3 <- left_join(emdat_dv, df_1, by = c("year", "iso3c"))
  
    # Add zeros
  df_3 <- df_3 %>%
    mutate(end_dis_ids = if_else(sapply(end_dis_ids, length) == 0, 
                              list(NA_character_), 
                              end_dis_ids),
      dis_death = replace_na(dis_death, 0))
  
  return(df_3)
}

# Process each dataset
EM_dat_1k_processed_4   <- process_EmDat_4(EM_dat_1k, EM_dat_1k_processed_3)
EM_dat_5k_processed_4   <- process_EmDat_4(EM_dat_5k, EM_dat_5k_processed_3)
EM_dat_50k_processed_4  <- process_EmDat_4(EM_dat_50k, EM_dat_50k_processed_3)
EM_dat_100k_processed_4 <- process_EmDat_4(EM_dat_100k,EM_dat_100k_processed_3)

summary(EM_dat_50k_processed_4)

check3 <- nrow(EM_dat_50k_processed_4[!is.na(EM_dat_50k_processed_4$end_dis_ids),]) # 846 disaster-endyears (states only)
...
process_ICEWS_3 <- function(df_EMdat, df_ICEWS_input ) {
...
df_ICEWS_EMDAT_1ky   <-process_ICEWS_3(EM_dat_1k_processed_4,df_ICWES_dis1k_y)  
df_ICEWS_EMDAT_5ky   <-process_ICEWS_3(EM_dat_5k_processed_4,df_ICWES_dis5k_y)
df_ICEWS_EMDAT_50ky  <-process_ICEWS_3(EM_dat_50k_processed_4,df_ICWES_dis50k_y)
df_ICEWS_EMDAT_100ky <-process_ICEWS_3(EM_dat_100k_processed_4,df_ICWES_dis100k_y)
...
cummean_ICEWS <- function(df) {
  
  #sort df
  df <- df[order(df$year), ]
  
  # Cumulative mean by row
  df$cum_mean_NormAgr <- cumsum(df$NormAgr) / seq_len(nrow(df))

  # grab last per year
  out <- aggregate(cum_mean_NormAgr ~ year, data = df,
                      FUN = function(x) tail(x, 1))
  
  out[, c("year", "cum_mean_NormAgr")]
}

# Summarize each dataset
df_ICWES_dis1k_y_small   <- cummean_ICEWS(df_ICEWS_EMDAT_1ky)
df_ICWES_dis5k_y_small   <- cummean_ICEWS(df_ICEWS_EMDAT_5ky)
df_ICWES_dis50k_y_small  <- cummean_ICEWS(df_ICEWS_EMDAT_50ky)
df_ICWES_dis100k_y_small <- cummean_ICEWS(df_ICEWS_EMDAT_100ky)
...
yearmean_ICEWS  <- function(df_input) {
...
# Summarize each dataset
df_ICWES_dis1k_y_small_b   <- yearmean_ICEWS(df_ICEWS_EMDAT_1ky)
df_ICWES_dis5k_y_small_b   <- yearmean_ICEWS(df_ICEWS_EMDAT_5ky)
df_ICWES_dis50k_y_small_b  <- yearmean_ICEWS(df_ICEWS_EMDAT_50ky)
df_ICWES_dis100k_y_small_b <- yearmean_ICEWS(df_ICEWS_EMDAT_100ky)
...
# Function to summarize data by year
process_ICEWS_5 <- function(df_input_2, df_input_3, df_input) {
  
  ## merge with emdat country month level dataset in order to identify declarations 
df_new= left_join(df_input, df_input_2, by = c("year"="year"))
df_new= left_join(df_new, df_input_3, by = c("year"="year"))

df_new$dummy_agreement_roll<-ifelse(df_new$NormAgr> df_new$cum_mean_NormAgr,1,0)
...
# Summarize each dataset
df_ICWES_dis1k_y   <- process_ICEWS_5(df_ICWES_dis1k_y_small, df_ICWES_dis1k_y_small_b, df_ICEWS_EMDAT_1ky)
df_ICWES_dis5k_y   <- process_ICEWS_5(df_ICWES_dis5k_y_small, df_ICWES_dis5k_y_small_b, df_ICEWS_EMDAT_5ky)
df_ICWES_dis50k_y  <- process_ICEWS_5(df_ICWES_dis50k_y_small, df_ICWES_dis50k_y_small_b, df_ICEWS_EMDAT_50ky)
df_ICWES_dis100k_y <- process_ICEWS_5(df_ICWES_dis100k_y_small, df_ICWES_dis100k_y_small_b, df_ICEWS_EMDAT_100ky)
...
df_AID_EMDAT_1ky   <-process_AID(EM_dat_1k_processed_4,df_aid_sec)  
df_AID_EMDAT_5ky   <-process_AID(EM_dat_5k_processed_4,df_aid_sec)
df_AID_EMDAT_50ky  <-process_AID(EM_dat_50k_processed_4,df_aid_sec)
df_AID_EMDAT_100ky <-process_AID(EM_dat_100k_processed_4,df_aid_sec)


summary(df_AID_EMDAT_50ky)
...
##### Calculating average aid sent during natural disaster year (cumulative mean)
```{r}

 #Function to calculate the mean of current and previous years

process_AID_2 <- function(df) {
  
  # sort df
  df <- df[order(df$year), ]
  
  # Cumulative mean
  df$cumulative_mean <- cumsum(df$Hum_plus_ER_aid_tot) / seq_len(nrow(df))
  
  # grab last
  out <- aggregate(cumulative_mean ~ year, data = df,
                      FUN = function(x) tail(x, 1))
  
  # rename
  names(out) <- c("year", "cumulative_mean_Hum_plus_ERaid")
  out
}
...
df_new$dummy_Hplus_ERaid_roll<-ifelse(df_new$Hum_plus_ER_aid_tot> df_new$cumulative_mean_Hum_plus_ERaid,1,0)
...
df_output <- df_new[ , names(df_new) %in% c("iso3c",
                                                ...
                                                "cumulative_mean_Hum_plus_ERaid",
...
# Summarize each dataset
df_AID_dis1k_y    <- process_AID_3(df_AID_dis1k_y_small,df_AID_dis1k_y_small_b,df_AID_dis1k_y)
df_AID_dis5k_y    <- process_AID_3(df_AID_dis5k_y_small,df_AID_dis5k_y_small_b,df_AID_dis5k_y)
df_AID_dis50k_y   <- process_AID_3(df_AID_dis50k_y_small,df_AID_dis50k_y_small_b,df_AID_dis50k_y)
df_AID_dis100k_y  <- process_AID_3(df_AID_dis100k_y_small,df_AID_dis100k_y_small_b,df_AID_dis100k_y)
...
round(prop.table(table(df50k$GlobSol_yearmean)) * 100, 2)
...
## create path
folder_path <- file.path(path,"globsol_df50k_v2_1.rds")
## save file
saveRDS(df50k,folder_path)

## create path
folder_path <- file.path(path,"globsol_df5k_v2_1.rds")
## save file
saveRDS(df5k,folder_path)

## create path
folder_path <- file.path(path,"globsol_df100k_v2_1.rds")
## save file
...
## create path
folder_path <- file.path(desktop_path, main_folder_name,sub_folder_name,"globsol_20251125_df50k_2_1.rds")
## save file
saveRDS(df50k,folder_path)

## create path
folder_path <- file.path(desktop_path, main_folder_name,sub_folder_name,"globsol_20251125_df5k_2_1.rds")
## save file
saveRDS(df5k,folder_path)

## create path
folder_path <- file.path(desktop_path, main_folder_name,sub_folder_name,"globsol_20251125_df100k_2_1.rds")
## save file
saveRDS(df100k,folder_path)
