...
process_disaster_data <- function(df) {
...
# Process each dataset
df_ICWES_g2g_dis1k <- process_disaster_data(df_ICWES_g2g_dis1k)
df_ICWES_g2g_dis5k <- process_disaster_data(df_ICWES_g2g_dis5k)
df_ICWES_g2g_dis50k <- process_disaster_data(df_ICWES_g2g_dis50k)
...
# count events
# aggregate at country year month level
df_ICWES_g2g_dis_y<- df_ICWES_g2g_dis %>% 
  group_by(iso3c_target, year, event_text) %>%
  summarize(count_senders = n_distinct(source_country))


#reshape
df_ICWES_g2g_dis_y<-reshape2::dcast(df_ICWES_g2g_dis_y, iso3c_target+year~event_text)
...
column_names_mapping <- c(
  ...
  "Express intent to ease economic sanctions boycott or embargo" = "embargo_dis",
...
names(df_ICWES_g2g_dis_y) <- column_names_mapping[names(df_ICWES_g2g_dis_y)]

#list variables 
List<- colnames(df_ICWES_g2g_dis_y)[colnames(df_ICWES_g2g_dis_y) %!in% c("iso3c", "year")]

# assign 0s
df_ICWES_g2g_dis_y<-Nas_to_0(df_ICWES_g2g_dis_y,List)

## sum
df_ICWES_g2g_dis_y$NormAgr <- rowSums(df_ICWES_g2g_dis_y[c("cooperate_dis",
                                                           "econo_dis",
                                                           "material_aid_dis",
                                                           "economic_aid_dis",
                                                           "human_aid_dis",
...
```{r}
# calculate 2 years average

df_ICWES_g2g_dis_y_small <- df_ICWES_g2g_dis_y %>% 
  group_by(year) %>%
  summarize(year_mean_NormAgr = mean(NormAgr))


## calculate 2 years average
df_ICWES_g2g_dis_y_small <- df_ICWES_g2g_dis_y_small %>%
  arrange(year) %>%
  mutate(two_year_year_mean_NormAgr_avg = (lag(year_mean_NormAgr) + year_mean_NormAgr) / 2) %>%
  filter(!is.na(two_year_year_mean_NormAgr_avg)) %>%
  ungroup()
```

##### Calculating wich countries received more normative agreement than the 2 years average
```{r}
## merge with emdat country month level dataset in order to identify declarations 
df_ICWES_g2g_dis_y= left_join(df_ICWES_g2g_dis_y, df_ICWES_g2g_dis_y_small, by = c("year"="year"))

df_ICWES_g2g_dis_y$dummy_agreement<-ifelse(df_ICWES_g2g_dis_y$NormAgr> df_ICWES_g2g_dis_y$two_year_year_mean_NormAgr_avg,1,0)


# keep var of interest
df_ICWES_g2g_dis_y <- df_ICWES_g2g_dis_y[ , names(df_ICWES_g2g_dis_y) %in% c("iso3c",
                                                                              "year",
                                                                              "dummy_agreement",
                                                                              "two_year_year_mean_NormAgr_avg",
                                                                              "NormAgr")]
...
NB: This is crap, bus be redone
##### Calculating total amount of aid by sector commited worldwide per each year
```{r}
# calculate five years average

df_Aiddata_sectoral_small <- df_Aiddata_sectoral %>% 
  group_by(year) %>%
  summarize(year_mean_aid = mean(Humanitarian_Aid_tot))


## calculate 2 years average
df_Aiddata_sectoral_small <- df_Aiddata_sectoral_small %>%
  arrange(year) %>%
  mutate(two_year_year_mean_aid = (lag(year_mean_aid) + year_mean_aid) / 2) %>%
  filter(!is.na(year_mean_aid)) %>%
  ungroup()

## merge with emdat country month level dataset in order to identify declarations 
df_Aiddata_sectoral= left_join(df_Aiddata_sectoral, df_Aiddata_sectoral_small, by = c("year"="year"))

df_Aiddata_sectoral$dummy_aid<-ifelse(df_Aiddata_sectoral$Humanitarian_Aid_tot> df_Aiddata_sectoral$two_year_year_mean_aid,1,0)
...
```{r}
df= left_join(df_ICWES_g2g_dis_y, df_Aiddata_sectoral, by = c("year" = "year", "iso3c" = "iso3c"))

df <- df %>%
  mutate(GlobSol = case_when(
    dummy_agreement == 1 & dummy_aid == 1 ~ 1,
    dummy_agreement == 1 & dummy_aid == 0 ~ 2,
    dummy_agreement == 0 & dummy_aid == 1 ~ 3,
    dummy_agreement == 0 & dummy_aid == 0 ~ 4
  ))
...
```{r}
df<-left_join(df_ICWES_g2g_dis_y)
```
