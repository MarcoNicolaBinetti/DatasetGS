...
#Polity2<- read_excel("~/Documents/data/polity2/p5v2018.xls")
```
...
#file_path<-"~/Documents/data/emdat.dta"
## import  dataset
...
process_ICEWS_2 <- function(df) {
  ...
  ## aggregate at country year month level
  df<- df %>% 
    group_by(iso3c_target, year, event_text) %>%
    summarize(count_senders = n_distinct(source_country))
  
  
  #reshape
  df<-reshape2::dcast(df, iso3c_target+year~event_text)
  
  # rename variables
  ## Named vector of column name mappings
  column_names_mapping <- c(
    "iso3c_target" = "iso3c",
    "year" = "year",
    "Engage in symbolic act" = "eng_simb_dis",
    "Express accord" = "expr_acc_dis",
    "Express intent to cooperate" = "cooperate_dis",
    "Express intent to cooperate economically" = "econo_dis",
    "Express intent to engage in diplomatic cooperation such as policy support" = "diplo_dis",
    "Express intent to engage in material cooperation" = "material_coop_dis",
    "Express intent to provide economic aid" = "economic_aid_dis",
    "Express intent to provide humanitarian aid" = "human_aid_dis",
    "Express intent to provide material aid" = "material_aid_dis",
    "Make empathetic comment" = "make_emp_dis"
  )
  
  # Rename columns using named vector
  names(df) <- column_names_mapping[names(df)]
  ...
  ## sum
  df$NormAgr <- rowSums(df[c("cooperate_dis",
                             "econo_dis",
                             "material_aid_dis",
                             "economic_aid_dis",
                             "human_aid_dis",
...
# Replace values with NA for years before 1995
df_ICEWS_EMDAT[df_ICEWS_EMDAT$year < 1995, List] <- NA
...
```{r}
# calculate average Norm agree per year

# Function to summarize data by year
process_ICEWS_3 <- function(df_input) {
  df_output <- df_input %>%
    group_by(year) %>%
    summarize(year_mean_NormAgr = mean(NormAgr))
  return(df_output)
}
...
df_Aiddata_sectoral_small <- df_aid_sec %>% 
  group_by(year) %>%
  summarize(year_mean_Haid = mean(Humanitarian_Aid_tot, na.rm=T),
            year_mean_ERaid = mean(Emergency_Response_tot, na.rm=T),
            year_mean_Hum_plus_ERaid = mean(Hum_plus_ER_aid_tot, na.rm=T),
            year_mean_ln_Hum_plus_ERaid = mean(ln_Hum_plus_ER_aid_tot, na.rm=T))

## merge with emdat country month level dataset in order to identify declarations 
df_aid_sec= left_join(df_aid_sec, df_Aiddata_sectoral_small, by = c("year"="year"))

df_aid_sec$dummy_Haid<-ifelse(df_aid_sec$Humanitarian_Aid_tot> df_aid_sec$year_mean_Haid,1,0)
df_aid_sec$dummy_ERaid<-ifelse(df_aid_sec$Emergency_Response_tot> df_aid_sec$year_mean_ERaid,1,0)
df_aid_sec$dummy_Hplus_ERaid<-ifelse(df_aid_sec$Hum_plus_ER_aid_tot> df_aid_sec$year_mean_Hum_plus_ERaid,1,0)
df_aid_sec$dummy_lnHplus_ERaid<-ifelse(df_aid_sec$ln_Hum_plus_ER_aid_tot> df_aid_sec$year_mean_ln_Hum_plus_ERaid,1,0)
...
df_aid_sec <- df_aid_sec[ , names(df_aid_sec) %in% c("iso3c",
                                                      ...
                                                      "year_mean_Haid",
                                                      "year_mean_ERaid",
                                                      "year_mean_Hum_plus_ERaid",
                                                      "year_mean_ln_Hum_plus_ERaid")]
```
...
unocha_long <-pivot_longer(unocha,2:11, names_to="year", values_to="Hum_plus_ER_aid_tot") %>%
  ...
  group_by(year) %>% # Generate annual mean - divide by ratio from analysis in compare-aid-data.R
  mutate(year_mean_Hum_plus_ERaid = mean(Hum_plus_ER_aid_tot)/6.5) %>%
  ungroup() 
...
unocha_long <- unocha_long %>% 
  mutate(Emergency_Response_tot = NA,
         ...
         year_mean_Haid = NA,
            year_mean_ERaid = NA,
            year_mean_ln_Hum_plus_ERaid = NA,
            dummy_Haid = NA,
            dummy_ERaid = NA,
         dummy_lnHplus_ERaid = NA)
 
# Join to full aid data
df_aid_sec_joined<-rbind(df_aid_sec %>% filter(year<2014),unocha_long)


```

## attempt code var humanitarian
```{r}
...
saveRDS(df,"globsol_20240313.rds")
...
p <- ggplot(data=df_g, aes(x = Norm_scaled, y = aid_scaled)) +
 ...
 scale_x_continuous(breaks=c(min(df_g$Norm_scaled)-14,0,max(df_g$Norm_scaled)),
                     ...
                     limits=c(min(df_g$Norm_scaled)-14,max(df_g$Norm_scaled)+5)) +
 scale_y_continuous(breaks=c(0,1500000000),
                     labels=c("Mean","High")) + 
 scale_colour_brewer(palette="Set1",name="GlobSol",
...
lab<-df_g %>% 
  mutate(label=
           ifelse(year=="2008" & iso3c=="CHN","China 2008",
           ifelse(year=="2010" & iso3c=="HTI","Haiti Earthquake\n 2010",
           ifelse(year=="2006" & iso3c=="SDN","Sudan \nFloods 2006",
           ifelse(year=="2005" & iso3c=="ETH","Ethiopia \n2005", 
           ifelse(year=="2005" & iso3c=="USA","\nUSA Hurricane Katrina 2005", 
           ifelse(year=="2005" & iso3c=="ZMB","Zambia \nDrought \n2005",
           ifelse(year=="2005" & iso3c=="PAK","Pakistan \n2005",
                  ifelse(year=="1999" & iso3c=="TUR","Turkey \nEarthquake \n1999",
           ifelse(year=="2018" & iso3c=="SEN","Senegal Drought 2018",NA)))))))))) %>%
  filter(!is.na(label))
...
lab<-df_g %>% 
 ...
 ifelse(year=="2004" & iso3c=="IDN","Indonesia",
           ...
           ifelse(year=="2004" & iso3c=="LKA", "Sri Lanka",NA)))))) %>%
  filter(!is.na(label))

add<-data.frame(year=2004,
                Norm_scaled=6.4+20.44+1.44-2.56,
                aid_scaled=108619633-7145305-20972991-3750192,
                label="Boxing Day Earthquake")
lab2<-rbind(lab %>% select(c(year,Norm_scaled,aid_scaled,label)),add)                
                  
                  
p+geom_text_repel(data=lab2,aes(x = Norm_scaled, y = aid_scaled, label=label),force=30,max.overlaps = 20)+
  geom_point(data=subset(lab2,label=="Boxing Day Earthquake"))+
  geom_point(data=lab2,aes(x = Norm_scaled, y = aid_scaled),shape=1,size=3) + 
  ggtitle("Boxing day earthquake 2004")
...
p+geom_text_repel(data=lab2,aes(x = Norm_scaled, y = aid_scaled, label=label),force=5,max.overlaps = 20)+
  geom_point(data=subset(lab2,label=="Boxing Day \nEarthquake"))+
  geom_point(data=lab2,aes(x = Norm_scaled, y = aid_scaled),shape=1,size=3) + 
  ggtitle("Boxing day earthquake 2004 (2005 impact)")
