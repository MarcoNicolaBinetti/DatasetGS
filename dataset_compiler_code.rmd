...
df_Aiddata_sectoral_small <- df_aid_sec %>% 
  group_by(year) %>%
  summarize(year_mean_Haid = mean(Humanitarian_Aid_tot),
            year_mean_ERaid = mean(Emergency_Response_tot),
            year_mean_Hum_plus_ERaid = mean(Hum_plus_ER_aid_tot),
            year_mean_ln_Hum_plus_ERaid = mean(ln_Hum_plus_ER_aid_tot))
...
```{r}
df= left_join(df_ICWES_dis50k_y, df_aid_sec, by = c("year" = "year", "iso3c" = "iso3c"))
#drop NA
