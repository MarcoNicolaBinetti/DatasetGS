...
#desktop_path <- file.path("Users","katrenrogers","Documents")
```
...
```{r}
URL_df<-paste0(folder_path,"/", "df_ICWES.rds")
#URL_df<-"~/Documents/data/ICEWS/icews.rds"
...
# Fill in blank varibales for bind 
...
# Plot the dataset
p<-ggplot(df_g, aes(x = Norm_scaled, y = aid_scaled)) +
  geom_point(aes(colour = factor(GlobSol)))+
  labs(x = "Normative agreement", y = "Sharing Behaviour") +
 geom_vline(xintercept = 0, linetype = "dashed", alpha=0.5) +
 geom_hline(yintercept = 0, linetype = "dashed", alpha=0.5) +
  scale_x_continuous(breaks=c(min(df_g$Norm_scaled),0,max(df_g$Norm_scaled)),
                     labels=c("Low", "Mean","High"))+
  scale_y_continuous(breaks=c(min(df_g$aid_scaled),0,max(df_g$aid_scaled)),
                     labels=c("Low", "Mean","High"))+
  scale_colour_brewer(palette="Set1",
                     labels=c("Cosmopolitan","Liberal","Postcolonial","Realist"),
                     name="") +
...
p+geom_label(aes(100,100000000,label="China Earthquake 2008"),label.size=0, size=3)
...
ggplot(df_proportions, aes(x = year, y = proportion, fill = factor(GlobSol))) +
  ...
  scale_fill_manual(values = c("#FF9999", "#66CCCC", "#99CC66", "#FFCC66"), name = "GlobSol") +  # Manually set colors for each category
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))  # Tilting the x-axis labels by 45 degrees
```
