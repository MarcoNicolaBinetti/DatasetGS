...
create_map <- function(data, column, title, fill_color) {
  ggplot(data) +
    ...
    scale_fill_manual(
      values = c("TRUE" = fill_color, "FALSE" = "white", "NA" = "grey"), # Grey for countries not in dataset
      na.value = "grey"
    ) +
...
ggsave("GlobSol_Distribution.png", final_plot, width = 8, height = 6)
print(final_plot)
...
# Save or display the final plot
ggsave("AidData_Sample_Description.png", final_plot, width = 8, height = 6)
print(final_plot)
...
df_g <- df50k%>%
mutate(aid_scaled = extended_aid_tot-year_mean_extended_aid_tot,
...
p <- ggplot(data=df_g, aes(x = Norm_scaled, y = aid_scaled)) +
 geom_point(aes(colour = factor(GlobSol2))) +
 geom_text(aes(label=iso3c))+
 labs(x = "Identification", y = "Joint Action") +
...
#ggsave("./plots/labelled_scatter_20240910.png", width=8,height=5)
```
